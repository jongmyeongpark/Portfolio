...
class Dead_reckoning_class:
    def __init__(self):
        self.sub_gps = rospy.Subscriber("smc_2000/fix", NavSatFix, self.gps_callback,queue_size=1)
        self.sub_gps_vel = rospy.Subscriber("smc_2000/navpvt",NavPVT,self.gps_data_callback,queue_size=1)
        ...
        self.Percent_POS_X = 0.2698 # 0.271 
        self.Percent_POS_Y = 0.271  # 0.271 
        ...
        self.return_predict_pose = [0,0,0]
        return_predict_Heading = 0
        self.last_predict_pose = [0,0]
        self.return_predict_Heading_array = []

        self.steer_edit = 0
        self.erp_steer_count = 0
        self.start_steer = 0
    ...
    def erp_callback(self, data):
        ...
        if self.erp_steer_count == 0:
            # self.steer_edit = -self.steer
            self.start_steer = self.steer
            self.steer_edit = 0
            self.erp_steer_count = 1
        self.steer =  self.steer + self.steer_edit 
    ...
    #####################################
        ...
        # self.imu_x.append(imu.orientation.x)
        # self.imu_y.append(imu.orientation.y)
    ...
    ######################################
        ...
        self.imu_yaw = self.imu_yaw + self.IMU_HEADING_CORRECTION
    ...
    def IMU_VELOCITY(self):
        ...
        return self.IMU_VELOCITY_VEL,self.IMU_ACCEL_X_avg,self.imu_yaw,self.IMU_VELOCITY_dT,
    ...
    def Dead_reckoning(self,signal):
        ...
        if self.Dead_reckoning_init == 0:
            ...
            IMU_velocity,IMU_accel,IMU_heading,IMU_dt = self.IMU_VELOCITY()
            ENC_velocity,ENC_accel,ENC_dt,ENC_flag = self.ENC_VELOCITY() 
        ...
        ################################################################################################
            ...
            if self.Dead_reckoning_signal >=2:
                ...
                if self.Dead_reckoning_signal == 2:
                    ...
                    self.Percent_POS_X = 0.2698 # 0.271 
                    self.Percent_POS_Y = 0.2698  # 0.271 
                    self.last_predict_pose = GPS_POS

                    self.heading_timer = time.time()
                    self.last_heading = IMU_heading
                ...
                if self.HEADING_EDIT_array != []:
                    self.HEADING_EDIT_MEAN = np.mean(self.HEADING_EDIT_array)
                else:
                    self.HEADING_EDIT_MEAN = 0

                self.IMU_HEADING_CORRECTION = self.HEADING_EDIT_MEAN

                # if round(self.steer,1) != round(self.start_steer,1): 
                if round(self.steer,1) != 0:
                    theta_rad = math.radians(self.steer)
                    
                    L = 0.985
                    self.steer_plus = 1
                    # 회전 반경 계산
                    r = L / math.tan(theta_rad)
                    ...
                    #     self.steer_plus = 1
                    if self.steer > 26:
                        self.steer_plus = 1.15
                    else:
                        self.steer_plus = 1    
                        # 왼쪽 바퀴 속도에서 오른쪽 바퀴 속도 계산
                    V_opposite = ( self.EKF_ENC_IMU_VEL * (r - L / 2) / (r + L / 2) ) 
                else:
                    V_opposite = self.EKF_ENC_IMU_VEL

                self.EKF_ENC_IMU_VEL = (self.EKF_ENC_IMU_VEL +  V_opposite) / 2    
                # if V_opposite > self.EKF_ENC_IMU_VEL :
                #     self.EKF_ENC_IMU_VEL  = V_opposite
                # else:    
                #     self.EKF_ENC_IMU_VEL =  self.EKF_ENC_IMU_VEL 
                print("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAaAAAA",self.steer,"AAA",self.EKF_ENC_IMU_VEL,V_opposite,self.steer_plus)
                ...
                if self.Dead_reckoning_signal == 3:
                    self.PREDICT_INIT_POS[0] += PREDICT_MOVING_X *self.Percent_POS_X 
                    self.PREDICT_INIT_POS[1] += PREDICT_MOVING_Y *self.Percent_POS_Y  
                ...
                # self.return_predict_Heading = np.arctan2(PREDICT_POS_Y - self.last_predict_pose[1],PREDICT_POS_X - self.last_predict_pose[0])
                self.return_predict_Heading = IMU_heading - self.IMU_HEADING_CORRECTION 
                self.return_predict_pose = [PREDICT_POS_X,PREDICT_POS_Y,self.return_predict_Heading]
                self.return_predict_Heading_array.append(self.return_predict_Heading)
                self.last_predict_pose = [PREDICT_POS_X,PREDICT_POS_Y]
            ...
            if self.Dead_reckoning_signal == 4:
                ...
                plt.plot(self.DEAD_RECKOING_count_plot[:], self.return_predict_Heading_array[:], label='predict_heading', color='blue')
        ...
        return self.return_predict_pose
