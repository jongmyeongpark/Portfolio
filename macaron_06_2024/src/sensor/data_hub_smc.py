...
class SensorDataHub:
    def __init__(self):
        ...
        self.GPS_DATA_pos_heading_radians = 0.0
    ...
    def imu_callback(self, imu):
        ...
        # if self.GPS_DATA_pos_heading_radians != 0.0:
        ...
        # rospy.loginfo(f'frontback: {self.imu_yaw}')
    ...
    def heading_callback(self, head):
        ...
        if self.GPS_move_flag == 1:
            ...
            self.GPS_DATA_pos_heading_radians =  2* np.pi - (self.GPS_DATA_pos_heading_radians-np.pi/2) 

            if self.GPS_DATA_pos_heading_radians >= 2*np.pi: self.GPS_DATA_pos_heading_radians -= 2*np.pi
            if self.GPS_DATA_pos_heading_radians < 0: self.GPS_DATA_pos_heading_radians += 2*np.pi
    ...
    def localization_update(self, select_heading):
        ...
        # heading = self.loc_fusion.get_heading(x, y, self.imu_yaw, self.sub_gps_heading, select_heading) # 지금은 그냥 gps 헤딩을 그대로 쓰지만, imu가 추가된 해딩을 처리하는 클래스가 필요.
        self.pos.x = x #955920.9088
        self.pos.y = y #1950958.212

        # if self.GPS_DATA_velocity_kmh >= 10:
        #     self.pos.z = self.GPS_DATA_pos_heading_radians
        # else:    
        #     self.pos.z = self.imu_yaw #만해 8.96 //  K-city 8.56
        self.pos.z = self.imu_yaw #만해 8.96 //  K-city 8.56
        # self.pos.z = heading #만해 8.96 //  K-city 8.56
        # print(self.GPS_DATA_pos_heading_radians, heading)    
...
def main():
    ...
    while not rospy.is_shutdown():
        if time.time() - start_rate > 0.02: # 0.2초 간격으로 실행. 데이터 처리가 0.3초보다 빨라도 여유롭게 0.3초간격으로 실행하고, 더 늦으면 업데이트 되는대로 실행.
            # print("rate : "),
