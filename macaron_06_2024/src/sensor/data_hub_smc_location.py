...
class SensorDataHub:
    def __init__(self):
        ...
        self.GPS_heading_plot,self.heading_plot = [],[]
        self.count,self.count_plot = 0,[]

        self.GPS_init_flag = False
        self.GPS_check_array = [0,0,0]
    ...
    def heading_callback(self,nav_msg):
        ...
        self.GPS_heading = 5*np.pi/2 - np.radians(nav_msg.heading / 1e5)
        if self.GPS_heading > 2*np.pi:
            self.GPS_heading -= 2*np.pi

        if self.GPS_velocity > 0.5:
            self.GPS_check_array[2] = self.GPS_check_array[1]  
            self.GPS_check_array[1] = self.GPS_check_array[0]  
            self.GPS_check_array[0] = self.GPS_heading

            if abs(self.GPS_check_array[0] - self.GPS_check_array[1]) > abs(self.GPS_check_array[2] - self.GPS_check_array[1]):
                pass
            else:
                self.sub_gps_heading = self.GPS_check_array[0]
                # print("pssssss")

            # print("ssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss",self.GPS_check_array)
            self.pos_move = True
        else:
            self.pos_move = False
    ...
    def localization_update(self, select_heading):
        ...
        if self.imu_flag != False:
            self.heading = self.loc_fusion.get_heading(x, y, self.sub_imu, self.sub_gps_heading, select_heading,self.pos_flag) # 지금은 그냥 gps 헤딩을 그대로 쓰지만, imu가 추가된 해딩을 처리하는 클래스가 필요.
        ...
        # self.count += 1
        # self.GPS_heading_plot.append(self.GPS_heading)
        # self.heading_plot.append(self.heading)
...
def main():
    ...
    # plt.figure(2)
    # plt.plot(Data.count_plot[100:], Data.GPS_heading_plot[100:], label='GPS_heading', color='red')
    # plt.plot(Data.count_plot[100:], Data.heading_plot[100:], label='heading', color='blue')
    # plt.xlabel('X')
    # plt.ylabel('Y')
    # plt.title('GPS_data')
    # plt.show()
...
#         self.GPS_velocity = velocity_ms * 3.6
...
#         # heading_diff = abs(heading - self.imu_yaw)
#         # if heading_diff > np.rad2deg(90) or heading_diff < np.rad2deg(270):
...
#     #기본 설정
#     rospy.init_node('data_hub', anonymous=True)
#     # rate = rospy.Rate(10) #지금 코드에서는 안써도 됨
