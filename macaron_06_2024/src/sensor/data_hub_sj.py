...
class SensorDataHub:
    def __init__(self):
        ...
        self.erp_edit_imu = rospy.Subscriber('imu_yaw', Float32, self.imu_yaw_callback, queue_size=1)
    ...
    def imu_callback(self, imu):
        ...
        if self.GPS_DATA_pos_heading_radians != 0:
            if self.GPS_DATA_heading_degrees >= self.yaw:
                self.yaw_edit = self.GPS_DATA_pos_heading_radians - self.yaw
            else:
                self.yaw_edit = self.yaw - self.GPS_DATA_pos_heading_radians 

        self.yaw = self.yaw - self.yaw_edit
        ...
        self.count += 1
        self.IMU_heading_array.append(self.imu_yaw)
