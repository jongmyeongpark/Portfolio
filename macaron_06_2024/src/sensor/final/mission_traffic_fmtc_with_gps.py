...
class mission_traffic:
    ...
    def run(self, mode):
        ...
        # erp와 정지선 사이의 거리를 토대로 신호등 미션 구간 업데이트 (16m 앞 ~ 5m 뒤)
        if self.mode == 0: 
            candidate_stop_s = []
        ...
        if self.mode == 1:
            candidate_stop_s = []
            for idx, stop in enumerate(self.stop_s):
                if self.current_s <= stop:
                    candidate_stop_s.append(stop)
        ...
        # if self.traffic_s - current_s > 10000:
        #     self.Stop_State = 'Far'
        #     self.publish()
        print(f"distance :{self.traffic_s - current_s}")

        if 35 < self.traffic_s - current_s < 50:
            print(f"distance :{self.traffic_s - current_s}")
            distance = self.traffic_s - current_s
        ...
        if self.traffic_flag == 0:
            ...
            if (detect_min_gap < self.traffic_s - current_s < detect_max_gap):
