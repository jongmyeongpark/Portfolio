...
class DualWebcamObjectDetectionROS:
    def __init__(self, webcam1_id=0, webcam2_id=4):
        ...
        self.sub_sign_state = rospy.Subscriber('/stop_sign', String, self.state_callback, queue_size=1)
        self.pub_sign = rospy.Subscriber('/stop', Bool, self.stop_callback, queue_size=1)
    ...
    def yolo_detection_combined(self, img, img_time):
        try:
            ...
            for box in selected_boxes:
                ...
                # 첫 번째 텍스트는 이미지의 하단에서 약간 위로, 30픽셀 위에 배치
                text = f"State: {self.detect_sign}"
                cv2.putText(combined_img, str(text), (10, img.shape[0] - 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
                # 두 번째 텍스트는 이미지의 하단에서 10픽셀 위에 배치
                cv2.putText(combined_img, str(self.stop_sign), (10, img.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
