...
def get_trans_matrix_camera3d_to_image_h1(img_shape) -> np.ndarray:
    ...
    # 대각선 기준 
    fov = np.deg2rad(74)  #브리오 카메라 대각선 방향 fov값 65 78 90 3개중에 하나임  => 일단 90도로 #76
...
class LidarCameraCalibration:
    ...
    def process_calibration(self):
        ...
        if self.lidar_data is not None and self.camera_image is not None:  # LiDAR 데이터와 카메라 이미지가 모두 존재하는 경우
            ...
            # projected_points_left, projected_indices_left = project_lidar_to_screen(self.delivery_centroids, self.camera_image[:, :640, :], ((-90,90,0), (0.20, 0.52, 0.3), (6, -17.5, -2)))
            # projected_points_right, projected_indices_right = project_lidar_to_screen(self.delivery_centroids, self.camera_image[:, 640:, :], ((-90,90,0), (-0.20, 0.52, 0.3), (2, 16.6, 0)))

            projected_points_left, projected_indices_left = project_lidar_to_screen(self.delivery_centroids, self.camera_image[:, :640, :], ((-90,90,0), (0.12, 0.52, 0.3), (1, -19, -2)))
            projected_points_right, projected_indices_right = project_lidar_to_screen(self.delivery_centroids, self.camera_image[:, 640:, :], ((-90,90,0), (-0.12, 0.52, 0.30), (-1, 17.5, 0)))
