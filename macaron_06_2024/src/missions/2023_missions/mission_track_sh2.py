...
"""
collision_check :  M
[ERROR] [1696668126.533039]: bad callback: <function callback at 0x7fa5c0532f70>
Traceback (most recent call last):
  File "/opt/ros/noetic/lib/python3/dist-packages/rospy/topics.py", line 750, in _invoke_callback
    cb(msg)
  File "/home/macaron/catkin_ws/src/macaron_5/src/missions/mission_track_sh2.py", line 802, in callback
    curve_abs, curve_ = curve_sum(center_line)
  File "/home/macaron/catkin_ws/src/macaron_5/src/missions/mission_track_sh2.py", line 606, in curve_sum
...
def get_angle(point1,point2):
    if(point1[1]-point2[1])==0:
        ...
        angle=math.atan(abs(x_delta)/abs(y_delta))
        return angle

def dot_maker_only_left(point1,point2,radius):
    x_delta=point2[0]-point1[0]
    y_delta=point2[1]-point1[1]
    alpha=get_angle(point1,point2)
    if x_delta>0 and y_delta>0:
        way_x=point2[0]+radius*math.cos(alpha)
        way_y=point2[1]-radius*math.sin(alpha)
        return [way_x,way_y]
        ...
        way_x=point2[0]
        way_y=point2[1]-radius
        return [way_x,way_y]  
      
        
def dot_maker_only_right(point1,point2,radius):
    x_delta=point2[0]-point1[0]
    y_delta=point2[1]-point1[1]
    alpha=get_angle(point1,point2)
    if x_delta>0 and y_delta>0:
...
def find_intersection(segment1, segment2):
    ...
    incline1 = (y2 - y1)/(x2 - x1)
    incline2 = (y4 - y3)/(x4 - x3)

    c1 = y1 - incline1 * x1
    c2 = y3 - incline2 * x3 

    x_intersection = (c2 - c1) / (incline1 - incline2)
    y_intersection = incline1 * x_intersection + c1

    if min(x1, x2) - 0.2  < x_intersection < max(x1, x2) +0.2 and min(y1, y2) -0.2< y_intersection < max(y1, y2)+0.2 :
...
def line_offset_check(left_cones, right_cones):
    min_dis = 1000
    for i, point1 in enumerate(left_cones):
        for j, point2 in enumerate(right_cones):
            if distance(point1, point2) < min_dis:
                min_dis = distance(point1, point2)
    return min_dis
...
def callback_erp(data):
    global erp_speed
    erp_speed =  data.read_speed
...
def follower_choice(center_line):
    ...
    for p in center_line:
        if distance([0, 0], p) > look_distance and p[0] > 0:
            follow_point = p
            ld_flag = True
            break
...
def distance_filter_for_left(point_list):
    ...
    if filter_flag_d is False:
        ...
        if len(point_list) > 1:
            ...
            unique_points_to_remove = points_to_remove
            # point_list에서 삭제
...
def curve_sum(line):
    if len(line) > 1:
        ...
        total_curve = 0
        total_curve_ = 0
        for i in range(min((len(line) - 1), 21)): #best 13
            delta_x = abs(line[i][0] - line[i + 1][0])
...
def callback(input_rosmsg):
    ...
    for p in input_rosmsg.points:
        ...
        if p.z == 1:
            ...
            if  0.2 < p.x < 2 and -0.8 < p.y < -0.2 and dis < min_dis:
                collision_check = "R"
                min_dis = dis
            ...
            if 0.2 < p.x < 2 and 0.2 < p.y < 0.8 and dis < min_dis:
                collision_check = "L"
                min_dis = dis
    ...
    if detect_flag==True:
        ...
        final_left_cones=distance_sort(final_left_cones)
        final_right_cones=distance_sort(final_right_cones)
        final_left_cones = final_left_cones[1:]
        final_right_cones = final_right_cones[1:]
        # print(final_right_cones)
        center_line = []
        
        center_offset = round(line_offset_check(left_cones=final_left_cones, right_cones=final_right_cones)/2,1)
        if center_offset > 2 or center_offset < 1 :
            center_offset = ex_center_offset
        ex_center_offset = center_offset
        #중점 라인 생성
        if right_line_len > left_line_len:    
            # if curve_ > 0 :
            #     center_offset = offset/2 - 0.5
            # else : 
            #     center_offset = offset/2 + 0.5
            for i in range(len(final_right_cones)-1):
                center_line.append(dot_maker_only_left(final_right_cones[i+1],final_right_cones[i],center_offset))
                center_line.append(dot_maker_only_right(final_right_cones[i],final_right_cones[i+1],center_offset))

        else:
            # if curve_ < 0 :
            #     center_offset = line_offset/2 - 0.5
            # else : 
            #     center_offset = line_offset/2 + 0.5
            for i in range(len(final_left_cones)-1):
                center_line.append(dot_maker_only_right(final_left_cones[i+1],final_left_cones[i],center_offset))
                center_line.append(dot_maker_only_left(final_left_cones[i],final_left_cones[i+1],center_offset))
                
        remove_points = []
        intsec = []
        for i in range(int(len(center_line)/2-1)):
            l1 = [center_line[2*i],center_line[2*i+1]]
            l2 = [center_line[2*i+2],center_line[2*i+3]]
            # print(l2)
            
            intersection = find_intersection(l1,l2)

            if intersection:
                remove_points.append(center_line[2*i+1])
                remove_points.append(center_line[2*i+2])
                center_line.append(intersection)
                intsec.append(intersection)
        
        for p in remove_points:
            if p in center_line:
                center_line.remove(p)
                
                
        #콘과 너무 가까운 웨이포인트 지우기
        point_to_remove = []
        for p in center_line:
            for q in left_line + right_line:
                if distance(p,q) < 0.8:
        ...
        center_line,center_line_len=linear(center_line,offset=line_offset)
    ...
    #Test ver 4
    # lookahead_index = 4
    # decision_dis = sum(map(lambda x: abs(x), global_path.ry[lookahead_index - 3:lookahead_index + 3]))
    # steer1 = int(min(max(-0.8 * decision_dis + 1.0, 0.2), 0.6) * pp_steer + min(max(0.8 * decision_dis, 0.4), 0.8) * -dwa_steer)
    
    # if curve_abs < 35:
    #     steer2 = int(0.6 * pp_steer + 0.4 * -dwa_steer)
    # elif curve_abs > 45:
    #     steer2 = int(0.2 * pp_steer + 0.9 * -dwa_steer)
    # else:
    #     steer2 = int((6.667e-06 * curve_ ** 3 - 0.0009 * curve_ ** 2 + 0.03183 * curve_ + 0.0625) * pp_steer + \
    #                 (-1.167e-05 * curve_ ** 3 + 0.001875 * curve_ ** 2 - 0.07471 * curve_ + 1.378) * -dwa_steer)

    # steer = int(0.3 * steer1 + 0.7 * steer2)   
    ##########################################################################################################################

    steer = int(pp_steer * 0.5) + int(0.6 * -dwa_steer)
    ...
    if curve_abs > 15:
        if collision_check == "L":
            steer -= 500
            pp_steer -= 500
        elif collision_check == "R":
            steer += 500
            pp_steer += 500
       # if cnt_l == 0:
    #     steer += 300
    ...
    inter = list_marker_maker("inter", "map", 1, intsec, 100, 100, 100, 0.3)
    ...
    marker_array = marker_array_rviz(
        [left, right, center, arrow, cone_y, cone_b, text, outlier_dis, outlier_ransac, noise_data, appended_noise, inter])
...
cur_pub = rospy.Publisher('/cuv_abs', Int16, queue_size=1)
erp_speed_sub = rospy.Subscriber('/erp_read', erp_read, callback_erp, queue_size=1)
