...
class DWA:
    ...
    def generate_predict_point_global(self, x, y, velocity, steer, heading, mode=0):  # local 좌표계 에서 예측 점 좌표를 구해서 global 좌표로 return
        # 접선 이동 거리 (= 호의 길이로 사용할 예정)
        if mode == 0:
    ...
    def cost_function(self, pose, obs_xy, mode):
        ...
        if mode == 0: 
            ...
            if mode == 1: # mini_obs mode
                cost1 = abs(gp_separation/2.3) if -1.5 <= gp_separation <= 1.5 else abs(gp_separation)
            elif mode == 2: # u-turn mode
                cost1 = abs(gp_separation/2.3) if -4.0 <= gp_separation <= 0.1 else abs(gp_separation * 100)
    ...
    def DWA(self, x, y, heading, obs_xy=None, mode=0, current_s=0, cur_speed=0, dwa_mode=0):  # (차량의 x, y, heading), (장애물의 x, y)
        ...
        if dwa_mode == 0:
            ...
            for steer in dw[1]:
                future_pos = self.generate_predict_point_global(x, y, velocity, steer, heading, dwa_mode)
        ...
        # mini_obs mode
            ...
            if obs_xy is None or len(obs_xy) == 0 or obs_xy == [[0.0, 0.0]]:
                obs_xy = [[0.0, 0.0], [0.01, 0.01], [0.02, 0.02], [-0.01, -0.01], [-0.02, -0.02]]
                
            cost = []

            if current_s+52 < self.max_index:
                for i in range(current_s+1, current_s+51, 10):
                    gp_path.append([self.glob_path.rx[i], self.glob_path.ry[i], self.glob_path.ryaw[i]])
        
            else:
                for i in range(current_s+1, self.max_index, 10):
                    gp_path.append([self.glob_path.rx[i], self.glob_path.ry[i], self.glob_path.ryaw[i]])
            ...
            for i in range(len(gp_path)):
                ...
                if 0.0 < abs(obs_d) < 1.0:
                    gp_cost = self.max_cost
                    break
                gp_cost += ((self.obstacle_force - obs_d) / self.obstacle_force if obs_d < self.obstacle_force else 0) * self.mode_1_w2 # 15는 테스트 해봐야함
            ...
            for steer in dw[1]:
                future_pos = self.generate_predict_point(self.current_pose[0], self.current_pose[1], velocity, steer, heading)
                
                candidate_paths.append(future_pos)
                cost.append(self.cost_function(future_pos, obs_xy=obs_xy, mode=mode))

            for i in range(len(cost)):
                if cost[i] < 100:
                    if cost[i] <= best_cost:
                        best_cost = cost[i]
                        selected_path = candidate_paths[i]

            if best_cost != cost[0] and self.flag == 0:
                self.num = 0
                self.flag = 2
            elif best_cost == cost[0] and self.flag == 2:
                self.num += 1
                if 50 <= self.num:
                    self.flag = 1
