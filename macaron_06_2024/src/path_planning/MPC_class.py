...
class MPC:
    def __init__(self):
        ...
        self.ox, self.oy, self.oyaw = [0.0] * self.T, [0.0] * self.T, [0.0] * self.T

        # MPC parameters
        self.R = np.diag([0.01, 0.01])  # input cost matrix
        self.Rd = np.diag([0.01, 1.0])  # input difference cost matrix
        self.Q = np.diag([1.0, 1.0, 0.5, 0.5])  # state cost matrix
        self.Qf = np.diag([0.1, 0.1, 0.5, 5])  # state final matrix
        ...
        self.MAX_DSTEER = np.deg2rad(20.0)  # maximum steering speed [rad/s]
    ...
    def update_erp_state(self, x, y, v, yaw, delta):
        self.state.x = x
        self.state.y = y
        self.state.v = v/3.6
        self.state.yaw = yaw
    ...
    def activate(self, cx, cy, cyaw, sp, delta, dl=1.0):
        ...
        xref, self.target_ind, dref = self.calc_ref_trajectory(
                self.state, cx, cy, cyaw, sp, dl, target_ind)
