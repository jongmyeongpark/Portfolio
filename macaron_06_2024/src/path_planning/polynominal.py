...
class cubic_polynomial:
    ...
    def calc_point(self, s):
        ...
        if s <= sf:
            q_s = self.a0 + self.a1*(s - self.si) + self.a2*(s - self.si)**2 + self.a3*(s - self.si)**3
        else:
            q_s = self.qf
    ...
    def calc_kappa(self, s, rk):
        q = self.a0 + self.a1*(s - self.si) + self.a2*(s - self.si)**2 + self.a3*(s - self.si)**3
        q_d = self.a1 + 2*self.a2*(s - self.si) + 3*self.a3*(s - self.si)**2
        q_dd = 2*self.a2 + 6*self.a3*(s - self.si)
        one_minus_qrk = 1 - q*rk
        S = np.sign(one_minus_qrk)
        Q = sqrt(q_d**2 + one_minus_qrk**2)
...
'''
    def setqf(self, qf):
        self.qf = qf
        self.a2 = (3*(qf - qi) - 2*tan(dtheta)*self.ds) / self.ds**2
        self.a3 = (2*(qi - qf) + tan(dtheta)*self.ds) / self.ds**3
'''
class Frenet_path:
