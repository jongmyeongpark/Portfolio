...
# MPC parameters
R = np.diag([0.01, 0.05])  # input cost matrix
Rd = np.diag([0.01, 0.1])  # input difference cost matrix
Q = np.diag([1.0, 1.0, 0.5, 0.5])  # state cost matrix
Qf = np.diag([1, 1, 0.5, 1.0])  # state final matrix
...
def first_calc_nearest_index(state, cx, cy, cyaw, pind):

    dx = [state.x - icx for icx in cx[pind:(pind + 200)]]
    dy = [state.y - icy for icy in cy[pind:(pind + 200)]]
...
def linear_mpc_control_back(xref, xbar, x0, dref):
    """
    backward mode
    """
...
def iterative_linear_mpc_control(xref, x0, dref, oa, od, frontback='f'):
    """
    ...
    for i in range(MAX_ITER):
        ...
        if frontback == 'f':
            ...
            oa, od, ox, oy, oyaw, ov = linear_mpc_control_back(xref, xbar, x0, dref)
        try:
...
class StaticMPC:
    ...
    def change_frontback(self, fb='f'):
        self.frontback = fb
    ...
    def activate(self, cx, cy, cyaw, sp, dl=0.5):
        ...
        #     self.target_ind, _ = first_calc_nearest_index(self.state, cx, cy, cyaw, 0)
        ...
        oa, odelta, ox, oy, oyaw, ov = iterative_linear_mpc_control(
                xref, x0, dref, oa, odelta, self.frontback)
        # print(xref[0,:])
        # print(xref[1, :])
        di, ai = 0.0, 0.0
