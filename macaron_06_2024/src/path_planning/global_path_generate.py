...
GLOBAL_TXT = "manhae_dwa_1028.txt"#"kcity_main_2023.txt" # mode 0 : 저장할 txt 파일 이름, mode 1 : 저장된 txt파일 이름
GLOBAL_NPY = "manhae_dwa_1028"#"kcity_main_2023"
...
def main():
    ...
    if(mode == 2) :
        x = []
        y = []
        pl = path_list.Path_list()
        path_info = pl.get_path(WHERE)
        coords = path_info["coord"]

        for coord in coords :
            plt.plot(float(coord[0]), float(coord[1]), "r*",zorder = 3)
            plt.plot(float(coord[2]), float(coord[3]), "r*",zorder = 3)

            wp = GenerateWayPoints([float(coord[0]), float(coord[1])], [float(coord[2]), float(coord[3])], float(coord[4]))
        ...
        glob_path = global_path.GlobalPath(x=x,y=y)
        ...
        gp_name_npy = PATH_ROOT_NPY + path_info["path_name"]
