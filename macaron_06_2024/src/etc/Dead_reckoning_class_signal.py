    ...
    def Dead_reckoning(self,signal):
        ...
        ################################################################################################
            ...
            if self.Dead_reckoning_signal >=2:
                ...
                PREDICT_VELOCITY_X = (-sin(IMU_heading) * self.ENC_IMU_velocity + self.LAST_PREDICT_VELOCITY_X)/2 ##GPS_data_heading을 IMU_heading으로 바꿔보고도 실험해주세요!
                PREDICT_VELOCITY_Y = (cos(IMU_heading) * self.ENC_IMU_velocity + self.LAST_PREDICT_VELOCITY_Y)/2 ##GPS_data_heading을 IMU_heading으로 바꿔보고도 실험해주세요!
...
def main():
    ...
    while not rospy.is_shutdown():    
        ...
        if s > 0 and s < 10:
            D.Dead_reckoning(1) ##Dead_reckoning()만 실행시키면 됨 // 숫자 1은 헤딩값 보정, 2는 데드렉코닝 시작, 4는 데드렉코닝 종료 및 그래프(생략 예정), 5는 데드레코닝 초기화
        elif s >= 10: #음영 구역 진입 전 여유롭게 1를 시작해야하며, 음영구역 시작 전 2를 시작해야함
            D.Dead_reckoning(2)
        rate.sleep()
