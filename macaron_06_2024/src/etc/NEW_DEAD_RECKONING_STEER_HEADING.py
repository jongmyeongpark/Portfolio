...
class Position:
    ...
    def imu_callback(self,imu):
        ...
        # self.yaw -= 1.0527 + self.IMU_HEADING_CORRECTION
        self.yaw -= 1.0527 
        # self.yaw -= self.IMU_HEADING_CORRECTION
        if self.yaw  >= 2*np.pi:
            self.yaw  -= 2*np.pi
            # print("-----------",self.yaw)
        elif self.yaw  <= 0:
...
def main():
    ...
    while not rospy.is_shutdown():  
        ...
        print("----------------------------------------------------------------------",GPS_data_heading,"-------",IMU_heading,"------------------",KF_HEADING)
        ...
        if count % 2 == 0 and count <DEAD_RECKONING_START:
            KF_HEADING = kf_HEADING.update(GPS_data_heading)
        else:    
            KF_HEADING = kf_HEADING.update(IMU_heading)
        ...
        if count >=DEAD_RECKONING_START:
            ...
            if count == DEAD_RECKONING_START:
                ...
                print("QQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQ",GPS_POS,PREDICT_INIT_POS,POINT_PREDICT_INIT_POS)
                ...
                Percent_POS = 0.26 # 0.2505
                p.IMU_HEADING_CORRECTION = HEADING_EDIT

            print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
            PREDICT_VELOCITY_X = (sin(GPS_data_heading) * ENC_IMU_velocity + LAST_PREDICT_VELOCITY_X)/2 ##GPS_data_heading을 IMU_heading으로 바꿔보고도 실험해주세요!
            PREDICT_VELOCITY_Y = (cos(GPS_data_heading) * ENC_IMU_velocity + LAST_PREDICT_VELOCITY_Y)/2 ##GPS_data_heading을 IMU_heading으로 바꿔보고도 실험해주세요!
