...
def main():
    ...
    IMU_velocity_plot,IMU_accel_plot,IMU_heading_plot = [],[],[0,0]
    ...
    measurement_variance1 = 0.00001 #0.00001
    estimation_error1 = 0.0  # 초기 추정 오차
    ...
    GPS_data_heading_KF = 0
    GPS_data_heading_KF_array = []
    ...
    ################################################################################################
        ...
        GPS_data_heading_KF = kf_HEADING.update(GPS_data_heading)
        GPS_data_heading_KF_array.append(GPS_data_heading_KF)
        ...
        #####################################
        DEAD_RECKONING_START = 300
        if p.GPS_move_flag == 1 and count < DEAD_RECKONING_START and count > 200:
            if GPS_data_heading_KF > IMU_heading:
                HEADING_EDIT = GPS_data_heading_KF - IMU_heading
                ...
                HEADING_EDIT = IMU_heading - GPS_data_heading_KF
            ...
            HEADING_EDIT_MEAN = np.mean(HEADING_EDIT_array) ##########################################################값이 이상하면 부호 바꾸기
        #####################################

        # DEAD_RECKONING_START = 350
        if count >=DEAD_RECKONING_START:
            GPS_POS = p.GPS_POS()
            IMU_heading = IMU_heading_plot[count]
            if count == DEAD_RECKONING_START:
            ...
            # p.IMU_HEADING_CORRECTION = np.mean(HEADING_EDIT_MEAN)
            print(HEADING_EDIT,IMU_heading,GPS_data_heading)
    ...
    plt.plot(count_plot[start:], IMU_heading_plot[start:-2], label='IMU_heading', color='blue')
    plt.plot(count_plot[start:], GPS_data_heading_plot[start:], label='GPS_DATA_heading', color='orange')
    plt.plot(count_plot[start:], GPS_data_heading_KF_array[start:], label='GPS_data_heading_KF', color='black')
    plt.xlabel('count')
    plt.ylabel('heading')
    plt.title('heading')
